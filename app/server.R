...
restaurant_manhattan_new<-na.omit(restaurant_manhattan_new)
shinyServer(function(input, output,session) {
  ...
  output$map <- renderLeaflet({
    ...
    observeEvent(input$map_click, {
      ...
      park_within_range      <- park_new[(distCosine(c(clong,clat),park_new[,c("Longitude_average","Latitdue_average")])-park_new[,c("radius")]) <= input$click_radius,]
      restaurant_within_range<- restaurant_manhattan_new[distCosine(c(clong,clat),restaurant_manhattan_new[,c("Longitude","Latitude")]) <= input$click_radius,]

...
shinyServer(function(input, output,session) {
                  ...
                  input$check_index),{
                    output$monthly_plot <- renderPlotly({
                      ...
                      monthly_index <-statistics %>% filter(NEIGHBORHOOD%in%c(input$check_neighbor))%>%
                        select(NEIGHBORHOOD, mon, index = input$check_index) %>% group_by(mon, NEIGHBORHOOD) %>%
                        summarize(index=mean(index)) %>% mutate(index=round(index,0)) 
                      
                      monthly_index$mon <- as.factor(monthly_index$mon)
                      
                      monthly_plot <- monthly_index %>%
                        plot_ly(x = ~mon, y = ~index, color = ~NEIGHBORHOOD, type = 'bar') %>%
                        layout(title = "Monthly Number of Places/Events by Neighborhood",
                               xaxis = list(title = "Month"),
                               yaxis = list(title = ""),
                               paper_bgcolor = 'transparent', plot_bgcolor = 'transparent') %>%
                        layout(autosize = F, width = 830, height = 350)
  ...
  filtered <- reactive({
    ...
    filtered<-index.processedtype %>%
      ...
      rename(PRICE=`SALE PRICE`, `AREA(SqFt)`=`LAND SQUARE FEET`,ZIP=`ZIP CODE`)
    return(filtered)
  ...
  output$recom<- renderDataTable(
    ...
    filtered()%>%select(ZIP,ADDRESS,PRICE,`AREA(SqFt)`), 
  ...
  observeEvent(c(input$caption,input$range,input$prange),{
    ...
    # crime
    crime_score<-unlist(crime_mahattan_geo_info%>%
                          filter(Latitude>range[1],Latitude<range[2],Longitude>range[3],Longitude<range[4])%>%
                          count())[[1]]
    ...
    output$near<-renderDataTable({
      range1<-location_range(geocode(paste(input$caption,", New York"),source="google")$lon,
                            geocode(paste(input$caption,", New York"),source="google")$lat,
                            distance=1.5)
      ...
      house_sub<-houseallinfo_neighbor%>%
        filter(Latitude>range1[1],Latitude<range1[2],Longitude>range1[3],Longitude<range1[4],
